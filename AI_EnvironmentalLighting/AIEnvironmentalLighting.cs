...
namespace EnvironmentalLighting
{
    ...
    public class AIEnvironmentalLighting : BaseUnityPlugin
    {
        ...
        internal void Awake()
        {
            ...
            _sun_intensity_multipier = Config.Bind("Sunlight", "Intensity Multiplier", 1.0f, new ConfigDescription("Amount to multiply sun direct light intensity by before applying.", new AcceptableValueRange<float>(0, 1)));
            ...
            _sun_shadow_multipier = Config.Bind("Sunlight", "Shadow Strength Multiplier", 1.0f, new ConfigDescription("Amount to multiply sun direct light shadow strength by before applying.", new AcceptableValueRange<float>(0, 1)));
            _moon_intensity_multipier = Config.Bind("Moonlight", "Intensity Multiplier", 1.0f, new ConfigDescription("Amount to multiply moon direct light intensity by before applying.", new AcceptableValueRange<float>(0, 1)));
            ...
            _moon_shadow_multipier = Config.Bind("Moonlight", "Shadow Strength Multiplier", 1.0f, new ConfigDescription("Amount to multiply moon direct shadow strength by before applying.", new AcceptableValueRange<float>(0, 1)));
            ...
            _ambient_night_intensity_multiplier = Config.Bind("Ambient Nightlight", "Intensity Multiplier", 1.0f, new ConfigDescription("Amount to multiply ambient light intensity by before applying.", new AcceptableValueRange<float>(0, 1)));
            _ambient_night_sky_intensity_multiplier = Config.Bind("Ambient Nightlight", "Trilight Sky Intensity Multiplier", 1.0f, new ConfigDescription("Amount to multiply Trilight ambient light intensity by before applying.", new AcceptableValueRange<float>(0, 1)));
            _ambient_night_equator_intensity_multiplier = Config.Bind("Ambient Nightlight", "Trilight Equator Intensity Multiplier", 1.0f, new ConfigDescription("Amount to multiply Trilight ambient light intensity by before applying.", new AcceptableValueRange<float>(0, 1)));
            _ambient_night_ground_intensity_multiplier = Config.Bind("Ambient Nightlight", "Trilight Ground Intensity Multiplier", 1.0f, new ConfigDescription("Amount to multiply Trilight ambient light intensity by before applying.", new AcceptableValueRange<float>(0, 1)));
            _cloud1_intensity_multiplier = Config.Bind("Weather", "Light Clouds Intensity Multiplier", 1.0f, new ConfigDescription("Amount to adjust sun/moon light intensity when there are light clouds.", new AcceptableValueRange<float>(0, 1)));
            _cloud2_intensity_multiplier = Config.Bind("Weather", "Partly Cloudy Intensity Multiplier", 1.0f, new ConfigDescription("Amount to adjust sun/moon light intensity when it is partly cloudy.", new AcceptableValueRange<float>(0, 1)));
            _cloud3_intensity_multiplier = Config.Bind("Weather", "Mostly Cloudy Intensity Multiplier", 1.0f, new ConfigDescription("Amount to adjust sun/moon light intensity when it is mostly cloudy.", new AcceptableValueRange<float>(0, 1)));
            _cloud4_intensity_multiplier = Config.Bind("Weather", "Completely Cloudy Intensity Multiplier", 1.0f, new ConfigDescription("Amount to adjust sun/moon light intensity when it is completely cloudy.", new AcceptableValueRange<float>(0, 1)));
            _fog_intensity_multiplier = Config.Bind("Weather", "Foggy Intensity Multiplier", 1.0f, new ConfigDescription("Amount to adjust sun/moon light intensity when it is foggy.", new AcceptableValueRange<float>(0, 1)));
            _rain_intensity_multiplier = Config.Bind("Weather", "Rain Intensity Multiplier", 1.0f, new ConfigDescription("Amount to adjust sun/moon light intensity when it is raining.", new AcceptableValueRange<float>(0, 1)));
            _storm_intensity_multiplier = Config.Bind("Weather", "Storm Intensity Multiplier", 1.0f, new ConfigDescription("Amount to adjust sun/moon light intensity when it is heavily storming.", new AcceptableValueRange<float>(0, 1)));
            _torchColorRed = Config.Bind("Objects", "Fire Light Color Red", 1.0f, new ConfigDescription("Amount to scale the Red color element of fires in the game", new AcceptableValueRange<float>(0, 2)));
            _torchColorGreen = Config.Bind("Objects", "Fire Light Color Green", 1.0f, new ConfigDescription("Amount to scale the Green color element of fires in the game", new AcceptableValueRange<float>(0, 2)));
            _torchColorBlue = Config.Bind("Objects", "Fire Light Color Blue", 1.0f, new ConfigDescription("Amount to scale the Blue color element of fires in the game", new AcceptableValueRange<float>(0, 2)));
            _waistLampColor = Config.Bind("Objects", "Waist Lamp Color", new Color (1.0f, 0.6f, 0.2f, 1.0f), "Color Value to set waist lamps to");
            _playerLampColor = Config.Bind("Objects", "Player Lamp Color", new Color(1.0f, 0.56f, 0.09f, 1.0f), "Color Value to set player lamps and torches to");
            _waistLampShadows = Config.Bind("Objects", "Waist Lamp Shadows", LightShadows.None, "Should waist lamps cast shadows.  WARNING: can cause a performance drop");
            _playerLampShadows = Config.Bind("Objects", "Player Lamp Shadows", LightShadows.None, "Should player lamps cast shadows.  WARNING: can cause a performance drop");
            ...
            _waistLampShadowNearPlane = Config.Bind("Objects", "Waist Lamp Shadow Near Plane", 0.2f, "Near clip plane of waist lamp shadows, if enabled");
            _playerLampShadowNearPlane = Config.Bind("Objects", "Player Lamp Shadow Near Plane", 0.2f, "Near clip plane of player lamp shadows, if enabled");
        ...
        internal static void EnviroSky_Update(EnviroSky __instance)
        {
            ...
            {
                if (currentWeather == AIProject.Weather.Storm)
                    directLightIntensity *= _storm_intensity_multiplier.Value;
                else if (currentWeather == AIProject.Weather.Rain)
                    directLightIntensity *= _rain_intensity_multiplier.Value;
                else if (currentWeather == AIProject.Weather.Fog)
                    directLightIntensity *= _fog_intensity_multiplier.Value;
                else if (currentWeather == AIProject.Weather.Cloud4)
                    directLightIntensity *= _cloud4_intensity_multiplier.Value;
                else if (currentWeather == AIProject.Weather.Cloud3)
                    directLightIntensity *= _cloud3_intensity_multiplier.Value;
                else if (currentWeather == AIProject.Weather.Cloud2)
                    directLightIntensity *= _cloud2_intensity_multiplier.Value;
                else if (currentWeather == AIProject.Weather.Cloud1)
                    directLightIntensity *= _cloud1_intensity_multiplier.Value;
            }
